...
import { CheckCircle2, Circle, AlertCircle, RefreshCw, Brain, Layers, Link2, ShieldCheck, Wand2, GitBranch } from "lucide-react";

export type StageStatus = "idle" | "running" | "complete" | "error" | "retrying";
  ...
  return (
    <div
      className={cn(
        ...
        !isRunning && !isError && "border border-transparent",
        stage.status === "idle" && "opacity-30",
      )}
    >
      {/* Active stage glow background */}
      {isRunning && (
        <div className={cn("absolute inset-0 rounded-xl", meta.bg, "opacity-40")}
      ...
      {/* Right: breathing dots (active) or stage number (idle) */}
      <div className="z-10 shrink-0">
        {isRunning  && <BreathingDots />}
        {isDone     && <span className="text-[9px] text-emerald-400 font-semibold">✓</span>}
    ...
    <>
      ...
      <div className="space-y-2.5 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-foreground text-xs leading-tight">
              {isComplete ? "Planning complete" : activeStage
        ...
        <div className="space-y-0.5">
          {stages.map(s => <StageRow key={s.stage} stage={s} />)}
        </div>
        ...
        {planText && !planResult && (
          <div className="p-2.5 rounded-xl bg-muted/30 border border-border/40 font-mono text-[10px] text-muted-foreground leading-relaxed max-h-28 overflow-y-auto whitespace-pre-wrap">
            {planText}
            <span className="inline-block w-0.5 h-3 bg-primary ml-0.5 align-middle"
