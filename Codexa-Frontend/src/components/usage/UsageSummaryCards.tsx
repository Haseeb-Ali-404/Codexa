...
export function UsageSummaryCards({
  ...
  const costScopeLabel =
    scopeLabel.toLowerCase() === "all time" ? "all-time" : scopeLabel;
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
      {cards.map((c, i) => (
        ...
        >
          ...
          <div className="pointer-events-none absolute -right-10 -top-10 h-28 w-28 rounded-full bg-white/[0.06] opacity-60 blur-2xl transition-opacity duration-300 group-hover:opacity-100" />
          <div className="relative flex items-start gap-4">
            <div
              className={cn(
                "flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-background/65 ring-1 ring-border/60 shadow-inner shadow-white/5 transition-transform duration-300 group-hover:scale-105 dark:bg-white/[0.06]",
              )}
            >
